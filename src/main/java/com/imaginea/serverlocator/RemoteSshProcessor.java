...
public class RemoteSshProcessor implements Runnable, ApplicationConstants {
	...
	public RemoteSshProcessor(ConcreteTopologyPublisher parentTopologyPublisher,JSONObject instanceJsonProperties){
	...
	public void run() {
		...
		try {
			...
			((ChannelExec) channel)
					.setCommand("/sbin/ifconfig | grep 'inet addr' | awk -F\":\" '{print $2}' | awk 'BEGIN {ORS = \";\"} {print $1}'");
			...
			StringBuilder localIpAddrSeq = new StringBuilder();
			...
			while (true) {
				while (in.available() > 0) {
					...
					if (i < 0)
						break;
					for(int k=0; k < i; k++){
						localIpAddrSeq.append((char)tmp[k]);
					}
				}
				if (channel.isClosed()) {
					break;
			...
			StringBuilder netstatOPSeq = new StringBuilder();
			channel = session.openChannel("exec");
			((ChannelExec) channel).setPty(true);
			((ChannelExec) channel)
					.setCommand("sudo netstat -tnap | awk 'BEGIN {OFS=\",\"; ORS= \";\"} NR > 2 {print $4,$5,$6,$7}'");
			...
			while (true) {
				while (in.available() > 0) {
					...
					if (i < 0)
						break;
					for(int k=0; k < i; k++){
						netstatOPSeq.append((char)tmp[k]);
					}
				}
				if (channel.isClosed()) {
					break;
			...
			channel.disconnect();
			session.disconnect();
			parentTopologyPublisher.processNetStatRecs(netstatOPSeq.toString(), localIpAddrSeq.toString());			
